...
class PlugTS_P(nn.Module):
    def __init__(self, num_users, num_items, embedding_k=6, nu=1, lamdba=1):
        ...
        self.nu = nu
        self.lamdba = lamdba
        self.W = torch.nn.Linear(self.num_users, self.embedding_k, bias=False)
        self.H = torch.nn.Linear(self.num_items, self.embedding_k, bias=False)
        self.linear_1 = torch.nn.Linear(12, 16)
        self.linear_2 = torch.nn.Linear(16, 8)
        self.linear_3 = torch.nn.Linear(9, 1, bias=False)
        self.relu = torch.nn.ReLU()
        self.sigmoid = torch.nn.Sigmoid()
        self.softmax = torch.nn.Softmax()
        self.xent_func = torch.nn.BCELoss()
        self.U = lamdba * torch.tensor(np.ones((9)))
    ...
    def forward(self, x_user, x_item):
        ...
        h2 = self.linear_2(h1)
        h2 = self.relu(h2)
        h2 = torch.cat((h2, torch.Tensor(np.ones((h2.size()[0],1)))),1)
        ...
        for epoch in range(num_epoch):
            ...
            for idx in range(total_batch):
                ...
                if not torch.is_tensor(sub_x_user):
                    sub_x_user = torch.Tensor(sub_x_user)
                if not torch.is_tensor(sub_x_item):
                    sub_x_item = torch.LongTensor(sub_x_item)
    ...
    def predict(self, x_user, x_item):
        with torch.no_grad():
            ...
            if not torch.is_tensor(x_item):
                x_item = torch.LongTensor(x_item)
