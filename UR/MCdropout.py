...
class MCdrop(nn.Module):
    ...
    def predict(self, x_user, x_item):
        with torch.no_grad():
            ...
            #print("num of forward is :",self.N,";time cost is:",dtime)
            pred_all = []
            ...
            pred_exp = torch.normal(mean.view(-1), sigma.view(-1))   
            mean = mean.view(-1)
            pred_final = mean

            pred_final = self.sigmoid(pred_final)
            pred_exp = self.sigmoid(pred_exp)
            #print("max1:",max(pred_final),max(pred_exp))  
            threshold = 0.015 #config.threshold
            print("threshold >:",threshold)
            idx = pred_final > threshold
            num_false = (idx == False).sum().item()
            print("num_false:",num_false)
            pred_final[idx] = pred_exp[idx]

            #pred_final = self.sigmoid(pred_final)
            res_pred = pred_final.cpu().detach().numpy().flatten()
