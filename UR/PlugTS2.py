...
class PlugTS2(nn.Module):
    def __init__(self, num_users, num_items, embedding_k=6, nu=1, lamdba=1):
        ...
        self.U = lamdba * torch.eye(9).cuda()
    ...
    def fit(self, x_user, x_item, y, num_epoch=1, lamb=0, lr=0.01, batch_size=64):
        ...
        for idx in range(total_batch):
            ...
            #print(grad.shape)
            self.U += torch.sum(torch.bmm(grad.unsqueeze(2),grad.unsqueeze(2).permute(0,2,1)), dim=0) 
    ...
    def predict(self, x_user, x_item):
        with torch.no_grad():
            ...
            sigma = torch.sqrt(torch.sum(self.lamdba * self.nu *torch.bmm(torch.matmul(grad.unsqueeze(2).permute(0,2,1), torch.inverse(self.U)),grad.unsqueeze(2)).squeeze(2),dim=1)) #*15
            #print(sigma)
