...
class NeuralTS(nn.Module):
    ...
    def predict(self, x_user, x_item):
        ...
        pred = self.forward(x_user, x_item).view(-1)
        #endtime0 = time.time()
        ...
        def get_grad(v):
            return torch.autograd.grad(pred, self.parameters(),v,retain_graph=True)
        grad_outputs = torch.eye(num_sample).cuda()    
        grad_vmap = vmap(get_grad)(grad_outputs)

        final_grad = torch.cat([x.view(num_sample,-1) for x in grad_vmap],dim=1)
        sigma = torch.sqrt(torch.sum(torch.mul(final_grad,final_grad), dim=1))
        final_pred = torch.normal(pred.view(-1), sigma.view(-1))
