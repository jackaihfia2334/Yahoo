...
def process_𝜖(idx_user):
    ...
    #print(np.mean(ctr_final))
    #print(max(ctr_final))
    threshold = config.threshold
    #idx =  ctr_final < threshold
    idx =  ctr_final > threshold
    if tmp<epsilon:
        #print("max1:",max(ctr_final))
        #ctr_final[idx] = np.random.uniform(low=0,high=1,size=ctr_final.shape)[idx]  
        ctr_final[idx]=np.random.uniform(low=min(ctr_final),high=max(ctr_final),size=ctr_final.shape)[idx]
        #print("max2:",max(ctr_final))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-thre', '--threshold', type=float, default=0.06, help='score epsilon.')
    ...
    #print("threshold:",config.threshold,";epsilon",config.epsilon)  #显示到微秒
